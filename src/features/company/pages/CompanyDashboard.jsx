...
export default function CompanyDashboard() {
    ...
    const [jobFilters,     setJobFilters]     = useState(DEFAULT_FILTERS); // ← NEW
    const jobSearchMounted = useRef(false);
    const jobFiltersMounted = useRef(false);   // ← NEW
    ...
    // ── Helper: convert filter object → query-param object ───────────────────
    function buildFilterParams(filters) {
        const params = {};

        // Multi-select → comma-separated string; backend splits with $in
     ...
     * @param {number} page
     * @param {string} search
     * @param {object} filters  — the full filter object (DEFAULT_FILTERS shape)
     */
    const fetchJobs = useCallback(async (page = 1, search = "", filters = DEFAULT_FILTERS) => {
        if (!companyId) return;
        setJobsLoading(true);
        setJobsError(null);
        try {
            const filterParams = buildFilterParams(filters);
            const res = await companyService.getJobs(companyId, page, {
                search,
                ...filterParams,
            });
            const d  = res?.data;
    ...
    // ── Initial load ──────────────────────────────────────────────────────────
    useEffect(() => { fetchJobs(1, "", DEFAULT_FILTERS); },    [fetchJobs]);
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {
            setJobsPage(1);
            fetchJobs(1, jobSearch, jobFilters);
        }, 350);
        return () => clearTimeout(t);
    }, [jobSearch]); // eslint-disable-line

    // ── Debounced filter re-fetch (NEW) ───────────────────────────────────────
    useEffect(() => {
        if (!jobFiltersMounted.current) { jobFiltersMounted.current = true; return; }
        const t = setTimeout(() => {
            setJobsPage(1);
            fetchJobs(1, jobSearch, jobFilters);
        }, 200); // slightly shorter than search debounce — filters are explicit clicks
        return () => clearTimeout(t);
    }, [jobFilters]); // eslint-disable-line
    ...
    const handleJobsPage = (next) => {
        ...
        setJobsPage(p);
        fetchJobs(p, jobSearch, jobFilters); // ← pass current filters
    ...
    const handleCreateInterviewer = () => setShowCreateInterviewerModal(true);

    const handleInterviewerCreated = () => refreshAll();
    ...
    const refreshAll = () => {
        fetchJobs(jobsPage, jobSearch, jobFilters);
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto px-6 py-6">
                    ...
                    {showCreate && (
                        <CreateJobForm
                            companyId={companyId}
                            onSuccess={() => { setShowCreate(false); setActiveTab("Jobs"); fetchJobs(1, "", DEFAULT_FILTERS); }}
                    ...
                    {!showCreate && activeTab === "Jobs" && (
                        <JobsSection
                            ...
                            onRetry={() => fetchJobs(jobsPage, jobSearch, jobFilters)}
                            handleJobsPage={handleJobsPage}
                            // ── NEW: filter state lives in Dashboard, passed down ──
                            onFiltersChange={(newFilters) => setJobFilters(newFilters)}
                        />
                    )}
                    ...
                    {showAssignModal && (
                        <AssignInterviewerModal
                            ...
                            onClose={() => { setShowAssignModal(false); setAssigningApp(null); }}
                    ...
                    {showReviewModal && (
                        <FeedbackReviewModal
                            ...
                            onClose={() => { setShowReviewModal(false); setReviewingApp(null); }}
