...
const STATUS_CFG = {
  ...
  hired:       { label: "Hired",       bg: "bg-emerald-100", text: "text-emerald-700", dot: "bg-emerald-500" },
...
function JobStatusBadge({ status, active }) {
  if (status === "APPROVED" && !active) {
    return <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-500">Closed</span>;
  }
  const c = JOB_STATUS_CFG[status] || JOB_STATUS_CFG.PENDING;
  return <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${c.bg} ${c.text}`}>{c.label}</span>;
}
...
function JobTypePill({ type }) {
  ...
  return <span className={`text-xs px-2 py-0.5 rounded-md font-semibold ${map[type] || "bg-slate-100 text-slate-600"}`}>{type}</span>;
}
...
// ─── NAV ─────────────────────────────────────────────────────────────────────
const TABS = ["Overview", "Jobs", "Applications", "Interviews", "Analytics"];
...
export default function CompanyDashboard() {
  ...
  const [jobPagination, setJobPagination] = useState({ currentPage: 1, totalPages: 0, totalJobs: 0 });
  const [appPagination, setAppPagination] = useState({ currentPage: 1, totalPages: 0, totalApplications: 0 });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [jobsRes, appsRes] = await Promise.allSettled([
        companyService.getJobs(companyId, jobPage),
        companyService.getAllApplications(companyId, appPage),
      ]);

      if (jobsRes.status === "fulfilled") {
        const d = jobsRes.value?.data;
        setJobs(d?.jobs || []);
        setTotalJobs(d?.pagination?.totalJobs || d?.jobs?.length || 0);
        setJobPagination({
          currentPage: d?.pagination?.currentPage || 1,
          totalPages: d?.pagination?.totalPages || 1,
          totalJobs: d?.pagination?.totalJobs || 0
        });
      }
      if (appsRes.status === "fulfilled") {
        const d = appsRes.value?.data;
        setApplications(d?.applications || []);
        setTotalApps(d?.pagination?.totalApplications || d?.applications?.length || 0);
        setAppPagination({
          currentPage: d?.pagination?.currentPage || 1,
          totalPages: d?.pagination?.totalPages || 1,
          totalApplications: d?.pagination?.totalApplications || 0
        });
      }
    } catch {
  ...
  // ── Derived stats (computed from real API data) ───────────────────────────
  ...
  const funnelData = Object.entries({ applied, shortlisted, interview: interviews, hired, rejected })
    .map(([key, value]) => ({
  ...
  // Weekly buckets (real appliedAt timestamps)
  const weeklyTrend = (() => {
    const buckets = {};
    [...applications]
      ...
      .forEach(app => {
        ...
        if (!buckets[key]) buckets[key] = { week: key, applied: 0, shortlisted: 0, hired: 0 };
        buckets[key].applied++;
        if (app.status === "shortlisted") buckets[key].shortlisted++;
        if (app.status === "hired")       buckets[key].hired++;
      });
    return Object.values(buckets).slice(-6);
  })();

  // Per-job application counts (match on job._id from applications)
  ...
  const filteredApps = applications.filter(app => {
    ...
    const matchSearch = !q ||
      ...
      (app.job?.jobTitle      || "").toLowerCase().includes(q) ||
  ...
  const NAV = [
    ...
    { icon: "👥", label: "Applications", tab: "Applications",  badge: applications.length || null },
    ...
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }} className="flex h-screen bg-slate-50 overflow-hidden">
          ...
          <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
            {NAV.map(item => (
              <button key={item.tab} onClick={() => setActiveTab(item.tab)}
                className={`nav-hover w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all text-left ${activeTab === item.tab ? "bg-white/15 text-white font-semibold" : "text-slate-400 hover:text-slate-200"}`}>
                <span>{item.icon}</span>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-100 px-6 py-3.5 flex items-center gap-4 flex-shrink-0">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-slate-400 hover:text-slate-700 text-xl leading-none transition-colors">☰</button>
          <div>
            <h1 className="text-sm font-bold text-slate-900">{NAV.find(n => n.tab === activeTab)?.label || activeTab}</h1>
            <p className="text-xs text-slate-400">Company Dashboard · {companyUser.companyName || "Company"}</p>
          </div>
          <div className="ml-auto flex items-center gap-3">
            <button onClick={fetchData} title="Refresh data" className="text-slate-400 hover:text-indigo-600 transition-colors text-base font-bold">↻</button>
        ...
        {/* Error banner */}
        {error && (
          <div className="mx-6 mt-4 flex items-center gap-2 text-rose-700 bg-rose-50 border border-rose-200 rounded-xl px-4 py-3 text-xs font-medium">
            <span>⚠️</span> {error}
            <button onClick={fetchData} className="ml-auto text-xs font-bold text-rose-700 underline">Retry</button>
          </div>
        )}
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-0.5">Here's your hiring overview.</p>
                </div>
              ...
              {loading ? <Spinner /> : (
                <>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <StatCard icon="💼" label="Total Jobs Posted" value={totalJobs}
                      sub={`${activeJobs} active · ${pendingJobs} pending`} color="bg-indigo-50" />
                    <StatCard icon="👥" label="Total Applications" value={totalApps}
                      sub={`${applied} new, ${shortlisted} shortlisted`} color="bg-sky-50" />
                    <StatCard icon="📅" label="In Interview Stage" value={interviews}
                      sub={`${shortlisted} shortlisted`} color="bg-amber-50" />
                    <StatCard icon="✅" label="Hired" value={hired}
                      sub={`${rejected} rejected`} color="bg-emerald-50" />
                  </div>
                        ...
                        {funnelData.map((stage, i) => {
                          ...
                          return (
                            <div key={i} className="flex-1 text-center">
                              ...
                              <div className="rounded-xl mx-auto" style={{ height: `${Math.max(pct * 0.85, 10)}px`, background: stage.color, minHeight: "10px" }} />
          ...
          {activeTab === "Jobs" && (
            <div className="space-y-5">
              ...
              {loading ? <Spinner /> : filteredJobs.length === 0 ? (
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5 py-4 mt-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-semibold text-slate-700 px-3 py-1.5 bg-slate-100 rounded-lg">
                          Page 1
                        </span>
                        <button 
                          className="px-3 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          ...
          {activeTab === "Applications" && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-slate-500">{totalApps} total · showing {applications.length} on this page</p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                  ...
                  {["all", "applied", "shortlisted", "interview", "hired", "rejected"].map(s => {
                    ...
                    return (
                      <button key={s} onClick={() => setStatusFilter(s)}
                        className={`px-3 py-1.5 rounded-xl text-xs font-semibold capitalize transition-all border ${statusFilter === s ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"}`}>
                ...
                <div className="relative ml-auto">
                  ...
                  <input value={appSearch} onChange={e => setAppSearch(e.target.value)}
                    ...
                    className="bg-white border border-slate-200 rounded-xl pl-8 pr-4 py-2 text-xs text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-300 w-60" />
                </div>
              </div>

              {loading ? <Spinner /> : filteredApps.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
          ...
          {activeTab === "Interviews" && (
            <div className="space-y-5">
              ...
              {loading ? <Spinner /> : interviewApps.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 p-12 shadow-sm text-center">
                  <p className="text-4xl mb-3">📅</p>
                  <p className="text-sm font-semibold text-slate-700 mb-1">No candidates in interview stage</p>
                  <p className="text-xs text-slate-400 mb-4">Move candidates to "Interview" status from the Applications tab.</p>
            ...
            <div className="space-y-6">
              ...
              {loading ? <Spinner /> : (
                <>
                  {/* KPIs */}
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <StatCard icon="📬" label="Total Applications" value={totalApps}     sub="From API pagination" color="bg-indigo-50" />
                    ...
                    <StatCard icon="✅" label="Hire Rate"
                      value={applications.length > 0 ? `${Math.round((hired / applications.length) * 100)}%` : "—"}
                      sub={`${hired} of ${applications.length}`} color="bg-emerald-50" />
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                      ...
                      {weeklyTrend.length === 0 ? (
                        <EmptyState icon="📈" message="No application data to plot" />
                      ) : (
                        <ResponsiveContainer width="100%" height={220}>
                          <LineChart data={weeklyTrend}>
                            ...
                            <Line type="monotone" dataKey="hired"       stroke="#10b981" strokeWidth={2.5} dot={{ r: 3 }} name="Hired" />
                          </LineChart>
                        </ResponsiveContainer>
                      )}
                    </div>
                      ...
                      {funnelData.every(f => f.value === 0) ? (
                        ...
                        <div className="flex items-center gap-4">
                          <ResponsiveContainer width="55%" height={200}>
                            <PieChart>
                              <Pie data={funnelData.filter(f => f.value > 0)} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={3} dataKey="value">
                  ...
                  {/* Bar: apps per job (matched from current API page) */}
                  <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                    <h3 className="text-sm font-bold text-slate-800 mb-1">Applications per Job (Current Page)</h3>
                    <p className="text-[10px] text-slate-400 mb-4">Matched by job._id across fetched applications</p>
                    {appsPerJob.every(j => j.applications === 0) ? (
                      <EmptyState icon="📊" message="No cross-referenced application data on this page" />
                    ) : (
                      <ResponsiveContainer width="100%" height={220}>
