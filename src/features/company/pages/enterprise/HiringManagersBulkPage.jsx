...
export default function HiringManagersBulkPage() {
  const navigate = useNavigate();
  const [entries, setEntries] = useState([{ name: "", email: "" }]);
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState(null);

  const addRow = () => setEntries([...entries, { name: "", email: "" }]);
  ...
  const onSubmit = async () => {
    ...
    setResult(null);
    try {
      const res = await companyService.bulkCreateHiringManagers({ items: valid });
      setResult(res?.data || res);
      if ((res?.data?.summary?.failed || res?.summary?.failed || 0) === 0) {
        setTimeout(() => navigate("/company/enterprise/hiring-managers"), 2000);
      }
    } catch (e) {
  ...
  return (
    <div>
      ...
      <h2 className="text-xl font-semibold text-slate-900 mb-6">Bulk Add Hiring Managers</h2>

      <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        <table className="min-w-full text-sm">
          <thead><tr><th className="text-left py-2">Name *</th><th className="text-left py-2">Email *</th><th></th></tr></thead>
          <tbody>
            {entries.map((e, i) => (
              <tr key={i}>
