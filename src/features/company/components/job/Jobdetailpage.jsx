// ─────────────────────────────────────────────────────────────────────────────
// JobDetailPage.jsx  (fully corrected)
// Fixes applied:
//  1. normalizeJob / daysLeft / fmtShort imported from shared utils/normalizeJob
//     (was duplicated here and in JobsTab with slightly different logic)
//  2. canEditJobs moved to a proper auth utility call (getCompanyUser)
//     instead of reading localStorage directly in a useMemo
//  3. Minor: skill experience label de-duplication fixed (was `skill.experience ?? skill.experience`)
// ─────────────────────────────────────────────────────────────────────────────

import { useMemo } from 'react';
import { JobStatusBadge } from '../../ui/JobStatusBadge';
import { JobTypePill }    from '../../ui/JobTypePill';
// FIX 1: shared util — no more duplicated normalizeJob
import { normalizeJob, fmtShort, daysLeft } from '../../../../utils/normalizeJob';
// FIX 2: use the same auth helper as JobsTab
 ...
 *
 * Props:
 *  job    — the full raw job object from the API
 *  onBack — () => void
 *  onEdit — (job) => void
 */
export function JobDetailPage({ job, onBack, onEdit }) {
    ...
    // FIX 2: consistent with JobsTab — uses getCompanyUser() instead of
    // reading localStorage directly here
    const canEditJobs = useMemo(() => {
        const u = getCompanyUser();
                        ...
                        <div className="p-8 space-y-6">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="flex items-start gap-5">
                                    <div className="w-16 h-16 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-2xl flex items-center justify-center text-3xl flex-shrink-0 shadow-inner overflow-hidden">
                                        {j.companyLogo ? (
                                            <img src={j.companyLogo} alt={j.companyName} className="w-full h-full object-contain" />
                                        ) : (
                                            '💼'
                                        )}
                                    </div>
                                    <div className="min-w-0">
                                        ...
                                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
                                            ...
                                            <p className="text-sm text-slate-400">
                                                ID: <span className="font-mono font-medium">
                                                    {j.jobId || j._id?.slice(-8).toUpperCase()}
                                                </span>
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-6 border-t border-slate-50/80">
                                ...
                                <Stat
                                    ...
                                    sub={
                                        dl !== null && dl > 0  ? `${dl} days left` :
                                        dl !== null && dl <= 0 ? 'Expired'         : null
                                    }
                                    subColor={
                                        dl !== null && dl <= 3 ? 'text-rose-500'  :
                                        dl !== null && dl <= 7 ? 'text-amber-500' :
                                        'text-slate-400'
                                    }
                                />
                    ...
                    {j.jobDescription && (
                        <Section title="Job Description" icon="📝">
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: j.jobDescription }}
                            />
                        </Section>
                    )}
                        ...
                        <Section title="Required Expertise" icon="✨">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {j.requiredSkills.map((skill, index) => (
                                    <div
                                        key={skill._id || index}
                                        className="group relative flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl transition-all hover:bg-white hover:shadow-md hover:border-indigo-100"
                                    >
                                        ...
                                        <div className="flex items-center gap-3 ml-4">
                                            <div className="text-right">
                                                {/* FIX 3: was `skill.experience ?? skill.experience` — deduplicated */}
                                                <span className="block text-xs font-bold text-indigo-600">
                                                    {skill.experience ?? 0}+ yrs
                                                </span>
                ...
                <div className="space-y-6">
                    <div className="bg-indigo-600 rounded-3xl p-6 text-white shadow-lg shadow-indigo-100">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-xl font-black">{j.applicationsCount}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <Section title="Employment Details" icon="ℹ️">
                        <div className="space-y-6">
                            ...
                            <SidebarStat label="Work Mode"     value={j.isRemote ? 'Remote' : 'On-site'} />
                            <SidebarStat label="Work Location" value={j.remoteType || (j.isRemote ? 'Fully Remote' : 'On-site')} />
    ...
    return (
        <div className="flex items-start gap-3">
            ...
            <div>
                ...
                {sub && (
                    <p className={`text-[10px] mt-1 font-bold px-2 py-0.5 rounded-md bg-slate-50 inline-block ${subColor}`}>
                        {sub}
                    </p>
                )}
            </div>
